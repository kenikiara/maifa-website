...
$newOrders   = $db->query("SELECT COUNT(*) FROM orders WHERE status = 'pending'")->fetchColumn();
try {
    $newSubs = $db->query("SELECT COUNT(*) FROM subscribers WHERE DATE(created_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)")->fetchColumn();
} catch (\Exception $e) {
    $newSubs = 0;
}
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="orders.php" class="sidebar-link<?= isActive('orders.php', $current) ?>">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
      Orders
      <?php if ($newOrders > 0): ?>
      <span class="sidebar-badge"><?= $newOrders ?></span>
      <?php endif; ?>
    </a>
    ...
    <a href="subscribers.php" class="sidebar-link<?= isActive('subscribers.php', $current) ?>">
      ...
      Subscribers
      <?php if ($newSubs > 0): ?>
      <span class="sidebar-badge"><?= $newSubs ?></span>
      <?php endif; ?>
    </a>
